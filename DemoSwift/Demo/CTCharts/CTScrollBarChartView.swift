...
extension CTScrollBarChartView {
    ...
    private func initData() {
        ...
        /* 两个柱子的间距最小是 minSpacing, 等间距
         * 按两个柱子中线的最小间距计算出chartView的宽度。
         * 如果小于contentWidth，则使用contentWidth当做宽度，不可滚动
         * 否则可以滚动，使用计算出来的宽度
         */
        let width = CGFloat((barWidth + minSapcing) * Double(data.yAxisData.count) + minSapcing) + chartEdgeInset.left + chartEdgeInset.right
         ...
         * 计算所有bar的frame
         */
        let spacing = (contentWidth - chartEdgeInset.left - chartEdgeInset.right - CGFloat(barWidth) * CGFloat(data.yAxisData.count)) / CGFloat(data.yAxisData.count + 1)
        for (index, item) in data.yAxisData.enumerated() {
            let orignX = (CGFloat(barWidth) + spacing) * CGFloat(index) + spacing + chartEdgeInset.left
            let barHeight = CGFloat(item.number) / CGFloat(yMaxNumber) * (frame.height - chartEdgeInset.top - chartEdgeInset.bottom)
            let rect = CGRect(x: orignX, y: frame.height - chartEdgeInset.bottom - barHeight, width: CGFloat(barWidth), height: barHeight)
...
extension CTScrollBarChartView {
    ...
    private func drawTable() {
        ...
        drawTextLayer(text: leftText, rect: leftTextRect, alignmentModel: kCAAlignmentLeft)
        ...
        drawTextLayer(text: rightText, rect: rightTextRect, alignmentModel: kCAAlignmentRight)
        ...
        for (index, value) in yScaleData.enumerated() {
            ...
            drawTextLayer(text: String(value), rect: textRect, alignmentModel: kCAAlignmentLeft)
        }
...
extension CTScrollBarChartView {
    ...
    private func nearestBarIndexTo(_ point: CGPoint) -> Int? {
        ...
        if let index = intervals.index(where: { $0 == min }) {
            return index
        }
    ...
    private func drawExtensionInfoLayer(_ index: Int, tapPoint: CGPoint) {
        ...
        drawExtensionInfoTextLayer(text: stamp, rect: stampRect, alignmentModel: kCAAlignmentLeft)
    ...
    private func drawExtensionInfo(_ tapPoint: CGPoint, info: Array<String>) {
        ...
        drawExtensionInfoTextLayer(text: str, rect: rect, alignmentModel: kCAAlignmentLeft)
    }
